...
const firebaseConfig = {
  ...
  databaseURL: "https://zientax-b93cc-default-rtdb.asia-southeast1.firebasedatabase.app",
...
function closeModal(){ document.getElementById('taskModalOverlay').classList.remove('is-open'); document.getElementById('taskForm').reset(); }
...
function handleTaskFormSubmit(e){
  ...
  addDoc(colTasks, { subject, title, deadline, type, desc, status:'belum' })
    .then(()=>{ closeModal(); })
    .catch((err)=>{
      console.error(err);
      showDbError('Gagal menyimpan tugas baru ke database. Cek koneksi internet & konfigurasi Firebase.');
    })
...
function showCalDetail(iso){
  ...
  document.getElementById('calEventDetail').innerHTML = `<strong>${label}</strong>` +
    evs.map(e => `<div class="cal-event-row">
        ...
        ${e.custom ? `<button class="icon-btn icon-btn--tiny" data-action="delete-event" data-id="${e.id}" title="Hapus acara">&times;</button>` : ''}
      </div>`).join('');
...
const KAS_START = '2026-07';        // awal tahun ajaran 2026/2027
...
const STUDENTS = [
  { induk:'25.7.1007', nama:'ABDULLAH RIZKI JULIANO', jk:'L' },
  { induk:'25.7.1008', nama:'ADIL PAMBUDI', jk:'L' },
  { induk:'25.7.1009', nama:'ALMIRA SAFA PERMATA', jk:'P' },
  { induk:'25.7.1010', nama:'ALVIANA AYU WULANDARI', jk:'P' },
  { induk:'25.7.1011', nama:'ANASTA SEPTRIA AULIYANA', jk:'P' },
  { induk:'25.7.1012', nama:'ANNE ELLOK RAHMA MAULIDA', jk:'P' },
  { induk:'25.7.1013', nama:'ASHILLA PUTRI ROMADHONI', jk:'P' },
  { induk:'25.7.1014', nama:'AZIZAH NASWA NUR ALIVIA', jk:'P' },
  { induk:'25.7.1015', nama:'BUNGA CAMELIA SARI', jk:'P' },
  { induk:'25.7.1016', nama:'DHANI ARYO MAULANA', jk:'L' },
  { induk:'25.7.1017', nama:'DZAKI IBRAR MUKLISIN', jk:'L' },
  { induk:'25.7.1018', nama:'FAIZ DAAREN EL FATIH BIN AWALUDIN', jk:'L' },
  { induk:'25.7.1019', nama:'FATIN NADA SALSABILA', jk:'P' },
  { induk:'25.7.1020', nama:'GALUH CAHYA NINGRUM', jk:'P' },
  { induk:'25.7.1021', nama:'JAYINDRA KAKA ATMAJA', jk:'L' },
  { induk:'25.7.1022', nama:'KIRANA PARAHITA BILQIS AGTYASTA', jk:'P' },
  { induk:'25.7.1023', nama:'LISAN PATTI ARERAM', jk:'L' },
  { induk:'25.7.1024', nama:'MADA VOLTA AGATHON', jk:'L' },
  { induk:'25.7.1025', nama:'MOHAMMAD BILAL FAWAAZA', jk:'L' },
  { induk:'25.7.1026', nama:'MUHAMMAD AINUR ROFIQ', jk:'L' },
  { induk:'25.7.1027', nama:'MUHAMMAD FADIL', jk:'L' },
  { induk:'25.7.1028', nama:'MUHAMMAD REZA AFFADHIL', jk:'L' },
  { induk:'25.7.1030', nama:'NATAKA RADITYA AL FAQIH', jk:'L' },
  { induk:'25.7.1031', nama:'NATASHA ARUM WIJAYANTI', jk:'P' },
  { induk:'25.7.1032', nama:'PIVEL FAITH WIBAWA', jk:'L' },
  { induk:'25.7.1033', nama:'RAAFI YUSRAN', jk:'L' },
  { induk:'25.7.1034', nama:'REZA ALVINO OKTAVIANTO', jk:'L' },
  { induk:'25.7.1035', nama:'SELVIYATUL SHOLIKAH', jk:'P' },
  { induk:'25.7.1036', nama:'SHAFIRA MAULIA', jk:'P' },
  { induk:'25.7.1037', nama:'ZAHRA RAMADHANI', jk:'P' },
  { induk:'25.7.1038', nama:"ZAHRA' ZA'IIMAH ZAKIYYAH", jk:'P' },
];
  ...
  list.innerHTML = filtered.map(s=>{
    ...
    const monthsHTML = KAS_MONTHS.map(ym=>{
      ...
      return `<button type="button" class="kas-month-btn ${mClass} ${isCurrent ? 'is-current' : ''}"
        ...
        title="${kasMonthLabel(ym)} — ${paid ? 'Lunas' + (rec.paidAt ? ' · dibayar ' + rec.paidAt.split('-').reverse().join('-') : '') : 'Belum bayar'}">
        ${kasMonthShort(ym)}
      </button>`;
    }).join('');
